...
public class HeroDatabase extends SQLiteAssetHelper {
    ...
    private static HeroDatabase sInstance;
    private Context context;

    private HeroDatabase(Context context) {
    ...
    public static synchronized HeroDatabase getInstance(Context context) {
        if (sInstance == null)
            sInstance = new HeroDatabase(context);
    ...
    public void addHero(Hero hero, SQLiteDatabase db) {
        ...
        cv.put("damage_per_level", hero.getDamagePerLevel());
    ...
    public void addSpell(Spell spell, SQLiteDatabase db) {
        ...
        values.put("hotkey", spell.getHotkey());
    ...
    public void addTalent(Talent talent, SQLiteDatabase db) {
        ...
        values.put("tier", talent.getTier());
...
//                hero.setAttackRange(Double.parseDouble(cursor.getString(12)));
            ...
            hero.setDamagePerLevel(Double.parseDouble(cursor.getString(16)));
            ...
            if (spellCursor.moveToFirst()) {
                do {
                    ...
                    spell.setHotkey(spellCursor.getString(6));
            ...
            if (talentCursor.moveToFirst()) {
                do {
                    ...
                    talent.setTier(Integer.valueOf(talentCursor.getString(2)));
...
//                hero.setAttackRange(Double.parseDouble(allHeroCursor.getString(12)));
                ...
                hero.setDamagePerLevel(Double.parseDouble(allHeroCursor.getString(16)));
    ...
    public int getDatabaseVersion() {
        ...
        try {
            SQLiteDatabase db = this.getReadableDatabase();
            Cursor versionCursor = db.rawQuery("SELECT MAX(version) FROM updates", null);

            if (versionCursor.moveToFirst()) {
                version = versionCursor.getInt(0);
            }
    ...
    public void insertVersionUpdate(Version versionInfo) {
        ContentValues values = new ContentValues();

        values.put("version", versionInfo.getVersion());
        values.put("notes", versionInfo.getNotes());
        values.put("timestamp", versionInfo.getTimestamp());

        this.getWritableDatabase().insert("updates", null, values);
    }

    public void clearTables() {
        SQLiteDatabase db = sInstance.getWritableDatabase();
        db.execSQL("DELETE FROM heroes;");
        db.execSQL("DELETE FROM talents;");
        db.execSQL("DELETE FROM spells;");
